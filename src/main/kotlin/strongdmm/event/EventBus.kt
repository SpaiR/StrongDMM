...
object EventBus {
    ...
    fun <T, R> sign(eventClass: Class<out Event<T, R>>, eventAction: (Event<T, R>) -> Unit) {
        subscribersWithArg.getOrPut(eventClass) { mutableListOf() }.add(eventAction)
    }

    fun <T, R> sign(eventClass: Class<out Event<T, R>>, eventAction: () -> Unit) {
        subscribersWithoutArg.getOrPut(eventClass) { mutableListOf() }.add(eventAction)
    }

    @Suppress("UNCHECKED_CAST")
    fun <T, R> post(event: Event<T, R>) {
